...
class CalibrationDataGenerator:
    ...
    def overconfident_neural_network(
        ...
        y_pred = np.where(true_probs > 0.5, 
    ...
    def imbalanced_binary_classification(
        ...
        pos_mask = y_true == 1
        y_pred[pos_mask] = np.random.beta(2, 1, np.sum(pos_mask)) * (1 - bias_strength) + bias_strength
    ...
    def weather_forecasting_pattern(
        ...
        for i in range(1, n_samples):
            base_signal[i] = (temporal_correlation * base_signal[i-1] + 
                            np.sqrt(1 - temporal_correlation**2) * np.random.normal(0, 1))
    ...
    def click_through_rate_pattern(
        ...
        base_rates[high_prob_mask] += np.random.uniform(0.1, 0.4, np.sum(high_prob_mask))
    ...
    def generate_dataset(
        ...
        **kwargs
        ...
        if pattern not in generators:
            raise ValueError(f"Unknown pattern '{pattern}'. Available patterns: {list(generators.keys())}")
    ...
    def get_pattern_info(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            'medical_diagnosis': {
                ...
                'parameters': ['disease_prevalence', 'test_sensitivity', 'test_specificity'],
...
def quick_test_data(pattern: str = 'overconfident_nn', n_samples: int = 100) -> Tuple[np.ndarray, np.ndarray]:
