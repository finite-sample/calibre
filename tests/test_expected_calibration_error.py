"""Reference and scenario tests for expected calibration error."""
...
def _uniform_ece_reference(
    y_true: np.ndarray,
    y_pred: np.ndarray,
    n_bins: int,
    sample_weight: np.ndarray,
) -> float:
    """Direct implementation of the published weighted L1 formula."""
    edges = np.linspace(0.0, 1.0, n_bins + 1)
    bin_id = np.searchsorted(edges[1:-1], y_pred, side="right")
    total_mass = float(np.sum(sample_weight))
    result = 0.0
    for index in range(n_bins):
        ...
        result += mass / total_mass * abs(mean_pred - mean_true)
    return result
...
def test_matches_independent_weighted_reference_formula():
    """Unequal observation weights affect both bin means and bin mass."""
    ...
    expected = _uniform_ece_reference(y_true, y_pred, 4, weight)

    actual = expected_calibration_error(y_true, y_pred, n_bins=4, sample_weight=weight)

    assert actual == pytest.approx(expected, abs=1e-15)


def test_matches_published_torchmetrics_l1_example():
    """The public TorchMetrics ECE example is an external reference fixture."""
    y_true = np.array([0, 0, 1, 1, 1], dtype=float)
    y_pred = np.array([0.25, 0.25, 0.55, 0.75, 0.75])

    assert expected_calibration_error(y_true, y_pred, n_bins=2) == pytest.approx(
        0.29, abs=1e-15
    )
...
def test_equal_and_opposite_errors_can_cancel_inside_a_bin():
    ...
    hidden = expected_calibration_error(y_true, y_pred, n_bins=10)
    resolved = expected_calibration_error(y_true, y_pred, n_bins=100)
...
def test_weight_scaling_and_integer_replication_are_invariant():
    ...
    scaled = expected_calibration_error(
        y_true, y_pred, n_bins=4, sample_weight=weight * 17
    )
    replicated = expected_calibration_error(
...
def test_bin_count_is_capped_at_positive_weight_sample_size():
    ...
    capped = expected_calibration_error(y_true, y_pred, n_bins=3)
    oversized = expected_calibration_error(y_true, y_pred, n_bins=1_000_000)
...
def test_rejects_values_outside_binary_probability_domains(y_true, y_pred, match):
    """ECE has binary-outcome and probability input semantics."""
    with pytest.raises(ValueError, match=match):
        expected_calibration_error(y_true, y_pred)
...
def test_rejects_empty_mismatched_and_multidimensional_inputs():
    ...
    with pytest.raises(ValueError, match="same shape"):
        expected_calibration_error(np.array([0.0]), np.array([0.2, 0.8]))
    with pytest.raises(ValueError, match="one-dimensional"):
        expected_calibration_error(np.array([[0.0], [1.0]]), np.array([[0.2], [0.8]]))


def test_bin_count_is_keyword_only():
    """ECE and RMSCE expose the same unambiguous bin-count calling convention."""
    call = cast("Any", expected_calibration_error)
    with pytest.raises(TypeError):
        call(np.array([0.0, 1.0]), np.array([0.2, 0.8]), 2)


def test_heldout_pre_post_and_resolution_controls():
    """Calibration improves held-out ECE while Brier rejects a collapsed model."""
    ...
    before = expected_calibration_error(eval_outcomes, eval_score, n_bins=5)
    after = expected_calibration_error(eval_outcomes, calibrated, n_bins=5)
    bad_error = expected_calibration_error(eval_outcomes, bad, n_bins=5)
    collapsed_error = expected_calibration_error(eval_outcomes, base_rate, n_bins=5)
