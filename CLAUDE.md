...
**Current Version**: see `version` in pyproject.toml (static, PEP 621; releases are cut by tagging the matching `vX.Y.Z`)
...
- `weighted_pava`, `monotone_projection`
...
- matplotlib is also in the `test` group (which `dev` includes), so plots tests
  always run and never skip. Its lower bound is stated in three places (`plots`
  extra, `test`, `docs`); PEP 735 groups cannot reference a project's own extras,
  so bump together.
...
- **cvxpy**: Test-only independent optimizer for numerical reference checks.
...
numbers in README.md and `docs/examples/benchmarks.rst`; both read its
...
python -m benchmarks.figures          # -> docs/_static/bench/
```
...
committed results go stale silently otherwise. `docs/_static/bench/headline.csv`
...
must reproduce `sklearn_isotonic` to 1e-12, `aggregate.py` refuses to summarize a
...
## CI/CD Configuration
- `.github/workflows/ci.yml` is a shim calling py-canon's `reusable-ci.yml`
- Test matrix: Python 3.12, 3.13, 3.14 on Ubuntu, macOS and Windows
- Lint job runs ruff check + format, pydoclint, pyright, and `preen check --strict` — all blocking
- A wheel job installs the built wheel in a clean environment and runs the tests against it
